...
namespace library_2121 {
	public partial class BorrowPage : Form {
		...
		void RefreshData(string card) {
			data.DataSource = Utils.ExecuteQuery("SELECT * FROM borrow WHERE 借书证号=@0 AND 还书时间 IS NULL", card).DefaultView;
		}
				...
				}
				{
					try {
						...
						var sqlR1 = Utils.ExecuteUpdate(
							String.Format(
								"INSERT INTO borrow(图书编号,书名,借书证号,姓名,借书日期,状态,罚款金额,还书日期) SELECT 图书编号,书名,借书证号,姓名,'{0}','否',0,'{1}' FROM book,reader WHERE book.图书编号=@0 AND reader.借书证号=@1",
